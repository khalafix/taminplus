...
const ProductItem = ({ props, banner, similarProductItem, specialOfferProduct,
     addItem , bannerProductDetails , allCategories}) => {
    ...
    return (
        <>
            <Head>
                <title>{props.seoTitle && props.seoTitle != "null" && props.seoTitle !== "undefined" ? props.seoTitle : props.productName}</title>
                <meta name="description" content={props.seoDescription && props.seoDescription !== "undefined" ? props.seoDescription : props.productName} />
                ...
                <div className="container">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                                ...
                                props.subCategoryName ?
                                    <li class="breadcrumb-item">
                                        <Link href={`/category/${getUrl(props.subCategoryName)}`}>
                                            <a>
                                                {/* {props.subCategoryName} */}
                                                {allCategories?.find(f => f.categoryName == removeDash(props.subCategoryName) || f.enName == removeDash(props.subCategoryName))?.categoryName}
                            ...
                            <li class="breadcrumb-item">
                                <Link href={`/product/category/${getUrl(props.subCategoryName)}/${getUrl(props.categoryName)}`}>
                                    <a>
                                        {/* {props.categoryName} */}
                                        {allCategories?.find(f => f.categoryName == removeDash(props.categoryName) || f.enName == removeDash(props.categoryName))?.categoryName}
                            ...
                            <li class="breadcrumb-item">
                                <Link href={`/product/categorybrand/${getUrl(props.subCategoryName)}/${getUrl(props.categoryName)}/${getUrl(props.brandName)}`}>
                                    <a>
                                        {props.brandName}
                                    </a>
                                </Link>
                     ...
                     <div className="row">
                        ...
                        {
                            props?.similarModel?.similarArtcle || props?.similarModel?.similarVideo ?
                                <>
                                    <div className="col-xl-9 mb-3">
                                        <Card>
                                            <Row>
                                                <Col xl={6}> <SliderForFile productName={props.productName} data={props} files={props.orginalFileAttachments} name="product-detail-slider" /></Col>
                                                <Col xl={6}> <ProductDetail hideImage={true} data={props} selectProduct={addProducts} /></Col>
                                            </Row>
                                        </Card>
                                ...
                                <>
                                    <div className="col-xl-6 col-lg-6 pr-5 pt-5 ">
                                        <SliderForFile   productName={props.productName}  data={props} files={props.orginalFileAttachments} />
                                    </div>
                                    <div className="col-xl-6 col-lg-6 ">
                                        <ProductDetail data={props} selectProduct={addProducts} />
                                    </div>
                                </>
                        }
                    ...
                    </div>
                    {
                        specialOfferProduct?.length > 0 ?
                            <>
                                <div className=" mt-2">
                                    <div className="col-12">
                                        <SpecialOfferProduct products={specialOfferProduct}  id={props.id} />
                                    </div>
                                </div>
...
export async function getStaticProps({ params }) {
    ...
    var similarProduct = await productServices.getUserSimilarProductByTitle(params.slug);
    var specialOfferProduct = await productServices.getSpecialOfferProductList(10);
    let bannerProductDetails = await bannerServices.getAll(PositionPlace.ProductDetails);
...
export async function getStaticPaths() {
    ...
    paths = paths.data.map(q => ({ params: { slug: `${getUrl(q).toString()}` } }))
    return {
        paths: paths,
        fallback: true,
