...
import BannerOfferForBrandProductCategory from 'components/banner/BannerOfferForBrandProductCategory';
...
import BrandsProductsSubCategoryFilterSidebar from 'components/filters/brands/BrandsProductsSubCategoryFilterSidebar';
...
export default function Blog({ posts, total, isNew, topVisited, topSale, brandId,
    hasInventory, title, productCategoryId, allCategories, brands,
    featuresData, featuresIds, featuresValues, featuresTitles, categoryId, banner, categories,
    products, categoriesFeatures, features, brand, category, productCategory, bannerBrandFeatures }) {
    ...
    const sortOrder = (type) => {
        ...
        router.push({
            pathname: `/brandcategory/${brand}/${category}/${productCategory}`,
            query: { ...model },
        });
    ...
    return (
        <>
            <Head>
                <title>   {allCategories?.find(f => f.categoryName == removeDash(productCategory) || f.enName == removeDash(productCategory))?.categoryName}</title>
                <meta name="description" content={allCategories?.find(f => f.categoryName == removeDash(productCategory) || f.enName == removeDash(productCategory))?.categoryName} />
                <link rel="canonical" href={`/brandsubcategory/${brandId}/${categoryId}`}></link>
            </Head>
                            ...
                            <ol class="breadcrumb">
...

                                {
                                    ...
                                    <li class="breadcrumb-item ">
                                        ...
                                        <Link
                                            href={`/brand/category/${category}/${brand}`}>
                                            <a>
                                ...
                                }
                                {
                                    ...
                                    <li class="breadcrumb-item  active" aria-current="page">
                                        {/* <a>
                                            {allCategories?.find(f => f.id == allCategories?.find(k => k.categoryName == removeDash(category) || k.enName == removeDash(category))?.parentId)?.categoryName}
                                        </a> */}
                                        {allCategories?.find(f => f.categoryName == removeDash(productCategory) || f.enName == removeDash(productCategory))?.categoryName}
                                ...
                                posts.length > 0 ?
                                    <>
                                        <>
                                            ...
                                            {/* {
                                                allCategories?.find(f => f.categoryName == removeDash(productCategory) || f.enName == removeDash(productCategory))?.remark &&
                                            ...
                                            <div className='container'>
                                                {
                                                    ...
                                                    <div class="row mb-3 box-remark">
                                                        <div className="col-xl-12 entry-content product-detail-slider">
                                                            <div dangerouslySetInnerHTML={{ __html: allCategories?.find(f => f.categoryName == removeDash(productCategory) || f.enName == removeDash(productCategory))?.remark }} className=""></div>
                                                        </div>
                                                    </div>
                                                }
                                            </div>
                    ...
                    modalType == "sort" ?
                        ...
                        <Modal.Body>
                            <FeaturesFilterSidebar
                                ...
                                featuresIds={featuresIds}
                                featuresValues={featuresValues}
                                featuresTitles={featuresTitles} featuresData={featuresData} posts={posts} currentPage={true} brandId={brandId} productCategoryId={productCategoryId} isDetail={false} categoryChangeHandler={categoryChangeHandler} />                        </Modal.Body>
                }
...
export async function getServerSideProps(context) {
    ...
    let banner = await bannerServices.getAll(PositionPlace.OfferForBrandProductCategory);

    let resultCategories = await productCategoryServices.getUserAllList();
    let categoriesFeatures = await productCategoryServices.getFeaturesBySubCategoryIdAndBrand(model.category, model.brand, model.productCategory); // count=10

    let bannerBrandFeatures = await bannerServices.getAll(PositionPlace.BrandFeatures);
    ...
    return {
        props: {
            ...
            brand: model.brand,
            allCategories: resultCategories.data,
            ...
            bannerBrandFeatures: bannerBrandFeatures.data,
