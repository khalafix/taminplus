...
const ProductList = ({ data, error }) => {
  return data.map((post) => {
    return <ProductThumb post={post} isBlog={true} />;
  });
};
...
export default function Blog({
  ...
  return (
    <>
                  ...
                  <li class="breadcrumb-item active" aria-current="page">
                    {
                      allCategories?.find(
                        (f) =>
                          f.categoryName == removeDash(category) ||
                          f.enName == removeDash(category)
                      )?.categoryName
                    }
                  </li>
                )}
              ...
              {!mobileDevice ? (
                <div className="sort-order text-center ">
                  <div class="" style={{ display: "contents" }}>
                    <span class="sort-order-item font-weight-bold">
                      <img
                        height={"20px"}
                        width={"20px"}
                        src={"/images/sortorder.png"}
                      />{" "}
                      <a onClick={() => sortOrder("")}> مرتب سازی: </a>{" "}
                    </span>{" "}
                    &nbsp; &nbsp;
                    <span
                      style={{ color: `${colorActiveTopVisited}` }}
                      class="sort-order-item "
                    >
                      <a onClick={() => sortOrder("topVisited")}>
                        پر‌‌بازدید ترین{" "}
                      </a>
                    </span>
                    &nbsp; &nbsp;
                    <span
                      style={{ color: `${colorActiveTopSale}` }}
                      class="sort-order-item "
                    >
                      <a onClick={() => sortOrder("topSale")}>پر‌‌فروش ترین </a>
                    </span>
                    &nbsp; &nbsp;
                    <span
                      style={{ color: `${colorActiveIsNew}` }}
                      class="sort-order-item "
                    >
                      <a onClick={() => sortOrder("isNew")}>جدید ترین</a>
                    </span>
                    &nbsp; &nbsp;
                  </div>
                </div>
                ...
                <div className="d-flex justify-content-center">
                  <div className="filter ">
                    <span
                      ...
                      class="sort-order-item font-weight-bold "
                    >
                      {" "}
                      <img
                        height={"20px"}
      ...
      >
        {modalType == "sort" ? (
          <Modal.Header> مرتب سازی بر اساس</Modal.Header>
...
export async function getServerSideProps(context) {
  ...
  let categories = await productCategoryServices.getUserByParentId(
    10000000,
    model.category
  ); // count=10

  let banner = await bannerServices.getAll(
    PositionPlace.OfferForProductCategory
  );
