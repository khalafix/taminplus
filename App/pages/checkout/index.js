...
import { Button, Card, Col, Form, Nav, Row, OverlayTrigger, Tooltip, Modal, Popover } from "react-bootstrap";
import { ServerFileIdentifier } from "constants/configs";
import { removeItem, addItem, clear, getAll, removeItemCount, getCount } from 'redux/actions/shoppingCardActions';
...
const Checkout = (props) => {
    ...
    const popover = (
        <Popover id="popover-basic">
            ...
            <Popover.Content style={{ width: "400px", height: "80px" }}>
                ...
                <div className="mt-2 font-weight-bold" style={{ color: "dark !importent", fontFamily: "SDF" }}>
                    <Link href={"/profile/orders"}><a href="#"> <span className="flaticon-checklist text-dark"> سفارش های من</span>  </a></Link>
                </div>

                <div style={{ fontFamily: "SDF" }} className="mt-2 font-weight-bold">
                    <a className="text-danger" onClick={() => handleShow()}>  <span >X</span> <span > حذف همه</span> </a>
                </div>
    ...
    const ActionButton = () => {
        if (!isMobile) {
            return <OverlayTrigger trigger="click" placement="top" overlay={popover}>
                <button style={{ marginRight: "53%" }} size="sm" class="btn btn-secondary"><span className="flaticon-planning-1"></span>   </button>
            </OverlayTrigger>
        } else {
            return <OverlayTrigger trigger="click" placement="top" overlay={popover}>
                <button style={{ marginRight: "49.25%" }} size="sm" class="btn btn-secondary"><span className="flaticon-planning-1"></span>   </button>
            </OverlayTrigger>
        }
    ...
    const addOrderWithPay = async () => {
        ...
        for (let index = 0; index < props?.items.length; index++) {
            ...
            model.items.push({ productId: element.id, itemCount: element.itemCount ? element.itemCount : 1 });
        }
        ...
        <>
            {
                ...
                <>
                                ...
                                <section className="price-section mt-4">
                                    <div className="container">
                                        ...
                                        <div className="d-flex justify-content-start">
                                            ...
                                            <h6 >
                                                سبد خرید شما ( {props?.items.map(q => q.itemCount).reduce((total, num) => (total + num))} کالا )
                                            </h6>
                                            {/* <ActionButton /> */}
                                                ...
                                                {props?.items.map(q => {
                                                    return (
                                                        ...
                                                        <>
                                                            ...
                                                            <li className="shopping-card-product-item  " style={{ direction: "ltr" }}>
                                                                <div className="shopping-card-list-item-pic w-25 ">
                                                                    <Link href={`/product/${getUrl(q.enTitle)}`}>
                                                                        <a href="">
                                                                            <img className="w-100" src={`${ServerFileIdentifier()}${q.coverFile}`} />
                                                                        </a>
                                                                    </Link>
                                                                </div>
                                                                ...
                                                                <div className="shopping-card-list-item-price text-dark  w-25">
                                                                    {
                                                                        q.itemCount <= 1 ?
                                                                            <div  >
                                                                                <div className="text-danger mr-3">{"1"}</div>
                                                                                <div class="input-group mb-3">
                                                                                    <span class="input-group-text font-weight-bold flaticon-add text-danger" onClick={() => AddItemCount(q)}></span>
                                                                                </div>
                                                                            </div>
                                                                            :
                                                                            <div  >
                                                                                <div class="input-group mb-3">
                                                                                    ...
                                                                                    <span class="input-group-text font-weight-bold flaticon-add text-danger" onClick={() => AddItemCount(q)}></span>
                                                                                    <input value={q.itemCount} type="text" class="input-group-form-control  input-styles-item text-center " disabled />
                                                ...
                                                <div className="">
                                                    ...
                                                    <Card style={{ width: '100%' }}>
                                                        <Card.Body>
                                                            ...
                                                            <Card.Text style={{ textAlign: "center", marginTop: "2%" }}>
                                                                <div className="row">
                                                                    <div className="col-12 font-weight-bold ">

                                                                        <label className="ml-3">  جمع سبد خرید:</label>
                                                                        <label className="">{props.items.map(q => q.discountedPrice ? q.discountedPrice * q.itemCount : q.price * q.itemCount).reduce((total, num) => (total + num)).toLocaleString()}<small className="mr-2">ریال</small></label>
                                                                    </div>
                                                                    {
                                                                        (props.items.map(q => q.price * q.itemCount).reduce((total, num) => (total + num))) - (props.items.map(q => q.discountedPrice ? q.discountedPrice * q.itemCount : q.price * q.itemCount).reduce((total, num) => (total + num))) > 0 &&

                                                                        <div className="col-12 font-weight-bold">
                                                                            <label className="ml-3">   سود شما از خرید:</label>
                                                                            <label>{((props.items.map(q => q.price * q.itemCount).reduce((total, num) => (total + num))) - (props.items.map(q => q.discountedPrice ? q.discountedPrice * q.itemCount : q.price * q.itemCount).reduce((total, num) => (total + num)))).toLocaleString()}<small className="mr-2">ریال</small></label>
                                                                        </div>
                                                                    }

                                                                    {
                                                                        koponResult?.percent ?
                                                                            <>
                                                                                <div className="col-12 font-weight-bold">
                                                                                    <label className="ml-3" >    مبلغ تخفیف کوپن: </label>
                                                                                    <label>
                                                                                        {koponResult?.percent ?
                                                                                            <>
                                                                                                <span>

                                                                                                    {(((props.items.map(q => q.discountedPrice ? q.discountedPrice * q.itemCount : q.price * q.itemCount).reduce((total, num) => (total + num))) * koponResult?.percent) / 100).toLocaleString()}
                                                                                                </span>
                                                                                            </>
                                                                                            : props.items.map(q => q.discountedPrice ? q.discountedPrice * q.itemCount : q.price * q.itemCount).reduce((total, num) => (total + num)).toLocaleString()
                                                                                        }
                                                                    ...
                                                                    <div className="col-12 font-weight-bold  text-danger ">
                                                                        <label className="ml-3">    مبلغ قابل پرداخت:</label>
                                                                        <label>{koponResult?.percent ?

                                                                            ((props.items.map(q => q.discountedPrice ? q.discountedPrice * q.itemCount : q.price * q.itemCount).reduce((total, num) => (total + num))) - (((props.items.map(q => q.discountedPrice ? q.discountedPrice * q.itemCount : q.price * q.itemCount).reduce((total, num) => (total + num))) * koponResult?.percent) / 100)).toLocaleString()
                                                                            : props.items.map(q => q.discountedPrice ? q.discountedPrice * q.itemCount : q.price * q.itemCount).reduce((total, num) => (total + num)).toLocaleString()

                                                                        }


                                                                            <small className="mr-2">ریال</small></label>
                                                                    </div>
                                                            ...
                                                            <Card.Text>
                                                                <div className="row li-Space" >
                                                                    ...
                                                                    <input style={{ fontFamily: "SDF" }} ref={koponInput} className="custom-form-control text-center " name="koponCode" placeholder="    کوپن تخفیف" />

                                                                    <button onClick={() => checkKopon()} type="button" class="btn btn-success"><span className="flaticon-wallet"> &nbsp;اعمال</span></button>
                                                            ...
                                                            <div className="row mt-3 li-Space">
                                                                ...
                                                                <button disabled={loadingAction} onClick={() => addOrderWithPay()} type="button" class="btn btn-danger ">  ثبت سفارش و پرداخت آنلاین </button>
                                                ...
                                                <div className=" mt-2 mb-2">
                                                    ...
                                                    <Card style={{ width: '100%' }}>
                                                        <Card.Body>
                                                            ...
                                                            <Card.Text style={{ textAlign: "", marginTop: "2%" }}>
                                                                ...
                                                                <div className="row font-weight-bold mt-1 ">
                                                                    ...
                                                                    <div className="col-11">
                                                                        ...
                                                                        <label className="">{userInfo?.postCode}</label>
                                                                ...
                                                                <div className="row font-weight-bold mt-1 ">
                                                                    ...
                                                                    <div className="col-11">
                                                                        ...
                                                                        <label className="ml-2 ">    آدرس محل تحویل:</label>
                                                                        <label className="">{userInfo?.deliveryAddress}</label>
                                                                ...
                                                                <div className="col-md-1 col-xs-1 col-sm-1">
                                                                    ...
                                                                    <div className="col-md-10 col-xs-10 col-sm-10 ">
                                                                        <button onClick={() => handleShowCustomerInfo()} type="button" class="btn btn-danger ">   تکمیل اطلاعات کاربری و تغییر آدرس تحویل</button>
                                                                    </div>
                                                                    <div className="col-md-1 col-xs-1 col-sm-1 ">
                                                                    </div>
                                                                </div>
                            ...
                            <>
                                ...
                                <div className="container mt-5">
                                    <div className="row">
                                        <div className="col-lg-12 col-md-12 col-sm-12 mx-auto">
                                            <Card style={{ width: '100%' }}>
                                                <Card.Img className="mx-auto" style={{ width: "400px", height: "400px" }} variant="top" src="/images/empty-cart.svg" />
                                                <Card.Body className="mx-auto">
                    ...
                    <form style={{ display: "none" }} id="formSubmit" name="input"
                    ...
                    >
                        <input
                            ...
                            // })}
                            className="form-control  main-filter-input font-weight-bold" type="text" id="RefId" name="RefId" />
                        <input
                            // ref={register({
                            //     required: true,

                            // })}
                            className="form-control  main-filter-input font-weight-bold" type="text" id="MobileNo" name="MobileNo" />
                        ...
                        <Modal.Body>
                            ...
                            <Row style={{ direction: "ltr" }}>
                                <Col lg={3}>
                                    ...
                                    <button type="button" onClick={() => removeAllItem()} class="btn btn-danger"> حذف همه</button>
                    ...
                    <Modal
                        size="lg"
                        show={showCustomerInfo} onHide={handleCloseCustomerInfo}
                        closeButton
                    >
                        <Modal.Header closeButton >
                        ...
                        <Modal.Footer>
                            <button type="button" onClick={() => handleCloseCustomerInfo()} class="btn btn-secondary">بازگشت</button>
