...
import { removeItem, addItem, getAll, removeItemCount } from 'redux/actions/shoppingCardActions';
            ...
            <div className="shopping-card-container" >
                <div className="shopping-card-title">
                    ...
                    <div className="shopping-card-list-item-actions">
                        <span style={{ backgroundColor: "white", color: "black" }} onClick={() => {
                            setIsCardClose(true)
                        }}>
                            X
                        </span>
                ...
                <div className="shopping-card-info" style={{ overflowY: "scroll", height: "350px" }}>
                    {props.items.length > 0 ? <ul className="shopping-card-list">
                        {props.items.map((item, index) => {
                            return (
                                <li className="shopping-card-product-item mb-2 mt-2">
                                    <div className="shopping-card-list-item-pic w-25" onClick={() => setIsCardClose(true)}>
                                        <Link href={`/product/${item.enTitle ? getUrl(item.enTitle) : getUrl(item.productName) }`}>
                                            <a href=''>
                                                <img className="w-100" src={`${ServerFileIdentifier()}${item.coverFile}`} />
                                            </a>
                                        </Link>
                                    </div>
                                    
                                    {
                                        ...
                                        <div  className='shopping-card-list-item-infos '>
                                                ...
                                                <div class="input-group mb-3">
                                                    <span class="input-group-text font-weight-bold flaticon-add text-danger" onClick={() => AddItemCount(item, index)}></span>
                                                </div>
                                            </div>
                                            :
                                            <div  className='shopping-card-list-item-infos'>
                                                <div class="input-group mb-3">
                                                    <span class="input-group-text font-weight-bold flaticon-add text-danger" onClick={() => AddItemCount(item, index)}></span>
                                                    <input value={item.itemCount} type="text" class="input-group-form-control  input-styles-item text-center " disabled />
                                                    <span class="input-group-text font-weight-bold flaticon-remove text-danger" onClick={() => MinesItemCount(item, index)}></span>
                                                </div>
                                            </div>
                                    }
                                    ...
                                    <div className="shopping-card-list-item-price w-80 text-dark mb-2">
                                        {
                                            item.discountedPrice ?
                                                <>
                                                    <span className="text-muted   font-weight-light"  ><strike> {item?.price?.toLocaleString()}  </strike> </span>
                                                    &nbsp;&nbsp;
                                    ...
                                    <div className="shopping-card-list-item-actions">
                                        <span className="" onClick={() => {
                                            ...
                                            <span style={{ marginBottom: "0%" }} className="flaticon-remove"></span>
                                        </span>
                                    </div>
                                </li>
                            )
...

                {
                    props.items.length > 0 ?
                        ...
                        !getCookie("token") ?
                            <div className="checkout-btn" style={{ marginBottom: mobileDevice == false ? "13%" : "28%" }}>
                                <button onClick={() => login()} href="#" className="cs-btn-one btn-gradient-color btn-md  w-75"> ورود و ثبت سفارش</button>
                            </div>
                            :
                            <div className="checkout-btn" style={{ marginBottom: mobileDevice == false ? "13%" : "28%" }}>
                                <button onClick={() => checkoutCheck()} disabled={props.items.length === 0} href="#" className="cs-btn-one btn-gradient-color btn-md  w-75"> ثبت سفارش</button>
                            </div>
