...
const Orders = () => {
    ...
    const getDate = async () => {
        ...
        const result = await orderServices.getDetailById();
        setData(result.data);
    ...
    const addOrderWithPay = async (dataValue) => {
        setLoadingAction(true);
        let model = {};


        model.koponCode = dataValue.koponCode;
        model.orderId = dataValue.id;

        let result = await userPaymentServices.addUserPayment(model);
                            ...
                            <Table striped bordered hover className="text-center">
                                <thead>
                                    <tr>
                                        ...
                                        <th> وضعیت سفارش  </th>
                                        <th>تاریخ ثبت </th>
                                        <th> عنوان کوپن تخفیف</th>
                                        <th>   درصد کوپن تخفیف	</th>
                                        <th>  مبلغ کوپن تخفیف - ریال	</th>
                                        ...
                                        tempData.map((item, index) => {
                                                return (
                                                    <tr>
                                                        ...
                                                        <td>{item.koponTitle ? item.koponTitle : '-'}</td>
                                                        <td>{item.koponPercent}</td>
                                                        <td>{item.koponAmount?.toLocaleString()}</td>
                                                        ...
                                                        <td>
                                                            ...
                                                            &nbsp;&nbsp;
                                                            {
                                                                (item.orderStatus == OrderStatus.Register || item.orderStatus == OrderStatus.PaymentFailed  ) &&
                                                                <button className="btn  btn-sm btn-success" onClick={() => addOrderWithPay(item)}> <span className="flaticon-online-money"></span></button>
