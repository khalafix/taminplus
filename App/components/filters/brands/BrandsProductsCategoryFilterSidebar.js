...
const FilterSidebar = ({ props }) => {
    ...
    useEffect(() => {
        // getData();
        let tempData = [];
        for (let index = 0; index < props?.categoriesFeatures?.length; index++) {
            const element = props.categoriesFeatures[index];
...

                {
                    ...
                    <Card className={`mt-3  font-weight-bold `}  >
                        <Card.Header style={{ background: "none" }}>
                            {data.length > 0 && data[0].featureTitle }

                        </Card.Header>
                        <Card.Body className='p-0'>
                            <div className="example-collapse-text">

                                <ListGroup variant="flush" style={{ height: "100%", backgroundColor: "white !importent" }}>
                                    {
                                        data.length > 0 && data.map((item) => {
                                            return (
                                                <ListGroup.Item>
                                                    ...
                                                    <Link
                                                        href={`/product/${getUrl(item.enTitle ? item.enTitle : item.productName)}`}>
...
const BrandsProductsCategoryFilterSidebar = (props) => {
...
export default BrandsProductsCategoryFilterSidebar;