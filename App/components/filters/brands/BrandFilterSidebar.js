...
const FilterSidebar = ({ props }) => {
    ...
    const getBrandsData = async () => {
        ...
        setBrands(result.data?.filter(f => f.key !=-1 && f.title != removeDash(props.brand) && f.enTitle != removeDash(props.brand)));
        setTempBrandItems(result.data.filter(f =>f.key !=-1 && f.title != removeDash(props.brand) && f.enTitle != removeDash(props.brand)));
    ...
    const ClickParentCategory = (id, show, e) => {

        // e?.stopPropagation();
        if (openCollapseCategory.find(f => f.id == id)) {
            setOpenCollapseCategory([...openCollapseCategory.filter(f => f.id != id), { parentKey: id, id: id, show: show ? !show : true }])
        }
        else {
            let temp = [];
            temp.push({ parentKey: id, id: id, show: show ? !show : true });
    ...
    return (
        <>
            <form onSubmit={handleSubmit(onSubmit)} >
                    ...
                    <Card className={`mt-3  font-weight-bold `}  >
                        ...
                        <Card.Body className='p-0'>
                            <div className="example-collapse-text">
                                ...
                                <ListGroup variant="flush" style={{ height: props?.categories.length > 6 ?"400px":"auto", overflowY: props?.categories.length > 6 ?"scroll" :"auto", backgroundColor: "white !importent" }}>

                                    {
                                        props?.categories.length > 0 && props?.categories.map((parentItem) => {
                                            return (
                                                <ListGroup.Item>
                                                    <div className=" mt-2 mr-2 ml-2 mb-2 d-flex justify-content-between collapse-div " onClick={() => ClickParentCategory(parentItem.id, openCollapseCategory.find(f => f.id == parentItem.id)?.show)} >
                                                        <Link href={`/brand/category/${getUrl(parentItem.enTitle ? parentItem.enTitle : parentItem.categoryName)}/${getUrl(props.brand)}`}><a className={`${selectedChangeCategory == parentItem.id ? `text-danger` : `text-dark`} font-weight-bold collapse-div`}> {`${parentItem.categoryName}`} </a></Link>
                                                        <h6 className={`flaticon-right-chevron-1 ${openCollapseCategory.find(f => f.id == parentItem.id)?.show ? "rotate-270" : "rotate-90"}`} ></h6>
                                                    </div>
                                                    ...
                                                    <>
                                                        ...
                                                        <Collapse in={
                                                            openCollapseCategory.find(f => f.id == parentItem.id)?.show ?
                                                                openCollapseCategory.find(f => f.id == parentItem.id)?.show : false
                                                                                ...
                                                                                return (
                                                                                    <div className='collapse-div mr-5 ml-2 mb-2 mt-2  '>
                                                                                        <>
                                                                                            ...
                                                                                            <Link href={`/brandcategory/${getUrl(props.brand)}/${getUrl(parentItem.enTitle ? parentItem.enTitle : parentItem.categoryName)}/${getUrl(childItem.enTitle ? childItem.enTitle : childItem.title)}`}><a className={`${selectedChangeCategory == childItem.key ? `text-danger` : `text-dark`}  collapse-div`}> {childItem.title} </a></Link>
                ...
                <Card className={`mt-3 mb-3  font-weight-bold `}  >
                    <Card.Header style={{ background: "none" }}>
                        <input style={{ width: "100%", borderRadius: "15px" }} ref={register({
                            required: false,

                        })} onChange={(e) => changeBrandTitle(e.currentTarget)} className="input-group-form-control  input-styles text-center" name="brandTitle" placeholder="جستجو برند ..." />
                    </Card.Header>
                    <Card.Body className='p-0'>
                        <div className="example-collapse-text">

                            <ListGroup variant="flush" style={{height: brands.length > 6? "400px":"auto", overflowY: brands.length > 6?"scroll":"auto", backgroundColor: "white !importent" }}>
...
const BrandFilterSidebar = (props) => {
