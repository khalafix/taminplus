...
const FilterSidebar = ({ props }) => {
    ...
    const getFeatures = async () => {
        ...
        const result = await productCategoryServices.getFeaturesByTitle(props.category);
    ...
    const getFeaturesWithProducts = async () => {
        ...
        const result = await productCategoryServices.getFeatureOptionByCategory(props.category);
                ...
                <Card className={`mt-3 font-weight-bold `}  >
                    ...
                    <Card.Body className='p-0'>
                        <div className="example-collapse-text">
                            ...
                            <ListGroup variant="flush" style={{ backgroundColor: "white !importent" }}>
                                    ...
                                    brands?.length > 0 && brands?.map((parentItem) => {
                                        return (
                                            <ListGroup.Item>
                                                ...
                                                <Link href={`/product/categorybrand/${props.subcategory}/${props.category}/${getUrl(parentItem.enTitle ? parentItem.enTitle : parentItem.title )}`}>
                                                    <a className='text-dark'> {parentItem.title}</a></Link>
                    ...
                    <Card className={`mt-3 font-weight-bold  `}  >
                        ...
                        <Card.Body className='p-0'>
                            <div className="example-collapse-text">
                                ...
                                <ListGroup variant="flush" style={{ backgroundColor: "white !importent", height: featureProductsData?.length > 6 ? "400px" : "auto", overflowY: featureProductsData?.length > 6 ? "scroll" : "auto" }}>

                                    {
                                        featureProductsData?.length > 0 && featureProductsData?.map((parentItem) => {
                                            return (
                                                <ListGroup.Item>
                                                    <div className=" mt-2 mr-2 ml-2 mb-2 d-flex justify-content-between collapse-div " onClick={() => ClickParentCategory(parentItem.key, openCollapseCategory.find(f => f.key == parentItem.key)?.show)} >
                                                        {/* <Link href={`/product/category/${getUrl(props.category)}/${getUrl(parentItem.enTitle ? parentItem.enTitle : parentItem.title)}`}><a className={`${selectedChangeCategory == parentItem.key ? `text-danger` : `text-dark`} font-weight-bold collapse-div`}> {`${parentItem.key}`} </a></Link> */}
                                                        {`${parentItem.key}`}
                                                    ...
                                                    <>
                                                        ...
                                                        <Collapse in={
                                                            ...
                                                            <div className="example-collapse-text">
                                                                ...
                                                                <ListGroup variant="flush" style={{ height: parentItem?.brands?.length > 3 ? "150px": "auto" , overflowY: parentItem?.brands?.length > 3 ? "scroll" : "auto"  }}>
                                                                    <>
                                                                        {

                                                                            parentItem?.brands && parentItem?.brands.map((childItem) => {
                                                                                return (
                                                                                    <div className='collapse-div mr-5 ml-2 mb-2 mt-2  '>
                                                                                        <>

                                                                                            <Link href={`/product/${getUrl(childItem.productEnName ? childItem.productEnName : childItem.productName )}`}><a className={`${selectedChangeCategory == childItem.key ? `text-danger` : `text-dark`}  collapse-div`}> {childItem.brandName} </a></Link>
