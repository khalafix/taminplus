...
const FilterSidebar = ({ props }) => {
    ...
    const getProductCategoriesData = async () => {
        const result = await productCategoryServices.getCategoryByParentId(props.category);
        setProductCategoriesData(result.data);

    };

    const getBrands = async () => {
        // const result = await productCategoryServices.getCategoryWithBrands(props.category);
                ...
                </Card>
                {
                    ...
                    <Card className={`mt-3 font-weight-bold  `}  >
                        <Card.Header style={{ background: "none" }}>
                            قطعات    {props.allCategories?.find(f => f.categoryName == removeDash(props.category) || f.enName == removeDash(props.category)).categoryName}
                        </Card.Header>
                        <Card.Body className='p-0'>
                            <div className="example-collapse-text">

                                <ListGroup variant="flush" style={{ backgroundColor: "white !importent" ,height:productCategoriesData?.length > 6 ? "400px" :"auto", overflowY:productCategoriesData?.length > 6 ? "scroll" :"auto"}}>
                                            ...
                                            return (
                                                <ListGroup.Item>
                                                    <div className=" mt-2 mr-2 ml-2 mb-2 d-flex justify-content-between collapse-div " onClick={() => ClickParentCategory(parentItem.key, openCollapseCategory.find(f => f.key == parentItem.key)?.show)} >
                                                        <Link href={`/product/category/${getUrl(props.category)}/${getUrl(parentItem.enTitle ? parentItem.enTitle : parentItem.title)}`}><a className={`${selectedChangeCategory == parentItem.key ? `text-danger` : `text-dark`} font-weight-bold collapse-div`}> {`${parentItem.title}`} </a></Link>
                                                                                ...
                                                                                return (
                                                                                    <div className='collapse-div mr-5 ml-2 mb-2 mt-2  '>
                                                                                        <>
                                                                                            ...
                                                                                            <Link href={`/product/categorybrand/${getUrl(props.category)}/${getUrl(parentItem.enTitle ? parentItem.enTitle : parentItem.title)}/${getUrl(childItem.enTitle ? childItem.enTitle : childItem.title)}`}><a className={`${selectedChangeCategory == childItem.key ? `text-danger` : `text-dark`}  collapse-div`}> {childItem.title} </a></Link>
                ...
                <Card className={`mt-3  font-weight-bold `}  >
                    ...
                    <Card.Body className='p-0'>
                        <div className="example-collapse-text">
                            ...
                            <ListGroup variant="flush" style={{ backgroundColor: "white !importent" , height:  brands.length > 6 ? "400px" :"auto", overflowY: brands.length > 6 ?"scroll" :"auto"  }}>
...
const ProductsSubCategoryFilterSidebar = (props) => {
...
export default ProductsSubCategoryFilterSidebar;