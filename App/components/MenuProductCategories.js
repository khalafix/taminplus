...
const Menu = ({ logoImage, active, data, hideMenu, clickHide, type = 1 }) => {
          ...
          {data && data.length > 0 ? data.map((item, index) => {
            return (
              ...
              <li onClick={(e) => e.stopPropagation()}
                id={`nav-Mega-Menu-Sub-menu-item-${item.key}`}
                onMouseEnter={() => setActiveCatSubMenuIndex(index)} className='rmm__nav-item  nav-sub-item' role="none">
                <Link
                  // href={`${link}${item.key}`} 
                  // as={`/${link}/${item.id}`}
                  href={type == 1 ? `/category/${getUrl(item.enTitle ? item.enTitle : item.title)}` :
                    item.key == -1 ?
                      '/brands'
                      :
                      `/brand/${getUrl(item.enTitle ? item.enTitle : item.title)}`
                        ...
                        {item.children.map(childItem => {
                          return (
                            <>
                              ...
                              <NavItem className='nav-sub-sub-item' role="none" key={childItem.key}>
                                <NavItemLink
                                  // id={`${childItem.key}`}
                                  role="menuitem"
                                  href={type == 1 ?
                                    `/product/category/${getUrl(item.enTitle ? item.enTitle : item.title)}/${getUrl(childItem.enTitle ? childItem.enTitle : childItem.title)}`
                                    : `/brand/category/${getUrl(childItem.enTitle ? childItem.enTitle : childItem.title)}/${getUrl(item.enTitle ? item.enTitle : item.title)}`}
                                >
                                  <span> {childItem.title}</span>
                                  <span className="flaticon-right-chevron-1 flip-h "></span>

                                </NavItemLink>
                                <div className='nav-sub-sub-item-container'>
                                  {
                                    childItem.children && childItem.children.map(childchildItem => {
                                      return <NavItem id={`${childchildItem.key}`} className='nav-sub-sub-sub-item'>
                                        <NavItemLink
                                          ...
                                          href={type == 1 ? `/product/categorybrand/${getUrl(item.enTitle ? item.enTitle : item.title)}/${getUrl(childItem.enTitle ? childItem.enTitle : childItem.title)}/${getUrl(childchildItem.enTitle ? childchildItem.enTitle : childchildItem.title)}` :
                                            `/brandcategory/${getUrl(item.enTitle ? item.enTitle : item.title)}/${getUrl(childItem.enTitle ? childItem.enTitle : childItem.title)}/${getUrl(childchildItem.enTitle ? childchildItem.enTitle : childchildItem.title)}`}
                                        >
                                          <div>
                                          ...
                                          <div className='nav-sub-sub-sub-item-container'>
                                            {
                                              childchildItem.children.map(childchildchildItem => {
                                                return <NavItemLink
                                                  // id={`${childchildchildItem.key}`}
                                                  href={`/product/${getUrl(childchildchildItem.enTitle ? childchildchildItem.enTitle : childchildchildItem.title)}`}
                                                >
                                                  <span> {childchildchildItem.text} {childchildchildItem?.symbolTitle}</span>
