...
import DynamicFromForUserOpinion from 'components/product/DynamicFromForUserOpinion';
import Modal from 'react-bootstrap/Modal';
import { userOpinionServices } from 'services/support/userOpinionServices';
var moment = require('jalali-moment');

const UserComments = ({ data ,userOpinionCount }) => {
  ...
  const getData = async () => {
    ...
    if (data?.id){
      const result = await userOpinionServices.getByProductId(data.id);
    ...
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title >دیدگاه خود را برای این محصول بنویسید </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <DynamicFromForUserOpinion productId={data.id} handleClose={handleClose} />
        </Modal.Body>
