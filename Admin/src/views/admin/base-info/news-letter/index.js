...
const Page = () => {
  ...
  const Create = async (values) => {
    setLoading(true);
    const result = await newsLetterServices.add(values);
    if (result.isSuccess) {
  ...
  const Update = async (values) => {
    setLoading(true);
    const result = await newsLetterServices.update(values);
    if (result.isSuccess) {
      message.success(result.message);
      setLoading(false);
      setOpenModal(false);
  ...
  const Delete = async (id) => {
    setLoading(true);
    const result = await newsLetterServices.delete(id);
    if (result.isSuccess) {
      message.success(result.message);
      setLoading(false);
      setOpenModal(false);
  ...
  return (
    <PageContainer title={`${intl.formatMessage({ id: "pages" })}`}>
      <CPCard bodyStyle={{ padding: 10 }}>
        <AdvanceTable
          ...
          apiBuilder={newsLetterServices.getAll}
          toolbar={ToolbarInit({
