...
const Page = () => {
  ...
  const Create = async (values) => {
    setLoading(true);
    const result = await socialMediaServices.add(values);
    if (result.isSuccess) {
  ...
  const Update = async (values) => {
    setLoading(true);
    const result = await socialMediaServices.update(values);
    if (result.isSuccess) {
      message.success(result.message);
      setLoading(false);
      setOpenModal(false);
  ...
  const Delete = async (id) => {
    setLoading(true);
    const result = await socialMediaServices.delete(id);
    if (result.isSuccess) {
      message.success(result.message);
      setLoading(false);
      setOpenModal(false);
  ...
  return (
    <PageContainer title={`${intl.formatMessage({ id: "socialMedia" })}`}>
      <CPCard bodyStyle={{ padding: 10 }}>
        <AdvanceTable
          ...
          apiBuilder={socialMediaServices.getAll}
          toolbar={ToolbarInit({
