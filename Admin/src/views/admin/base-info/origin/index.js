...
const Origin = () => {
  ...
  const renderTitleModal = (type) => {
    switch (type) {
      case "add":
        return intl.formatMessage({ id: "add" }) + ' ' + intl.formatMessage({ id: "listOrigin" });
      case "edit":
        return intl.formatMessage({ id: "edit" }) + ' ' + intl.formatMessage({ id: "listOrigin" });
      case "delete":
  ...
  const CreateOrigin = async (data) => {
    ...
    const result = await originServices.add(data);
    if (result.isSuccess) {
  ...
  const UpdateOrigin = async (data) => {
    ...
    data.isActive = data.status;
    const result = await originServices.update(data);
    if (result.isSuccess) {
      message.success(result.message);
      setLoading(false);
      setOpenModal(false);
  ...
  const DeleteOrigin = async (id) => {
    setLoading(true);
    const result = await originServices.delete(id);
    if (result.isSuccess) {
      message.success(result.message);
      setLoading(false);
      setOpenModal(false);
  ...
  return (
    <PageContainer title={`${intl.formatMessage({ id: "listOrigin" })}`}>
      <CPCard bodyStyle={{ padding: 10 }}>
        <AdvanceTable
          filters={FiltersInit()}
          columnsTable={TableInit({
            onChangeAction: handleOpenModal,
            deleteFeature: DeleteOrigin,
            parentLoading: loading,
          })}
          apiBuilder={originServices.getAll}
          toolbar={ToolbarInit({
