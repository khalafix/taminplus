...
import { jobOpportunityServices } from "services/base-Info/jobOpportunityServices";
...
const JobOpportunities = () => {
  ...
  const Create = async (data) => {
    ...
    const result = await jobOpportunityServices.add(data);
    if (result.isSuccess) {
  ...
  const Update = async (data) => {
    ...
    data.isActive = data.status;
    const result = await jobOpportunityServices.update(data);
    if (result.isSuccess) {
      message.success(result.message);
      setLoading(false);
      setOpenModal(false);
  ...
  const Delete = async (id) => {
    setLoading(true);
    const result = await jobOpportunityServices.delete(id);
    if (result.isSuccess) {
      message.success(result.message);
      setLoading(false);
      setOpenModal(false);
  ...
  return (
    <PageContainer title={`${intl.formatMessage({ id: "jobOpportunities" })}`}>
      <CPCard bodyStyle={{ padding: 10 }}>
        <AdvanceTable
          ...
          apiBuilder={jobOpportunityServices.getAll}
          toolbar={ToolbarInit({
