...
import { poWarrantyEvalutionService } from "services/purchase/poEvaluations/poWarrantyEvalutionService";
...
export const POWarrantyEvaluation = ({
  onCloseModal,
  parentLoading,
  currentData,
  companyId
  ...
  const getEvalutionById = async () => {
    ...
    let result = await poWarrantyEvalutionService.getPOWarrantyEvaluationForCompany(companyId);
    if (result.isSuccess) {
      setLoading(false)
      setData([...result.data]);
...
export default POWarrantyEvaluation;
