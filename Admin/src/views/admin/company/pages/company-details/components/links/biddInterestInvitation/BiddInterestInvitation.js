...
import { bidderInterestInvitationService } from "services/purchase/bidder-interest-invitation/bidderInterestInvitationService";
...
export const BiddInterestInvitation = ({
  onCloseModal,
  parentLoading,
  currentData,
  companyId
  ...
  const getData = async () => {
    ...
    let result = await bidderInterestInvitationService.getBidderInterestInvitationtForCompany(companyId);
    if (result.isSuccess) {
      setLoading(false)
      setData(result.data)
    }
      ...
      <>
        ...
        <Row>
          <Col xs={11} sm={11} md={11} style={{ margin: '0 1%' }}>
            <GenrateLinkUploader
              items={
                record?.invitationFiles?.concat( record?.companyFiles)
              }
...
const getColumnSearchProps = (dataIndex) => ({
  ...
  return (
    <div>
       <AdvanceTable
          rowKey="bidderInterestInvitationRequestAvlCompanyId"
...
export default BiddInterestInvitation;
