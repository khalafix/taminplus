...
import { poMidTermEvalutionService } from "services/purchase/poEvaluations/poMidTermEvalutionService";
...
export const POMidTermEvaluation = ({
  onCloseModal,
  parentLoading,
  currentData,
  companyId
  ...
  const getEvalutionById = async () => {
    setLoading(true)
    let result = await poMidTermEvalutionService.getPurchaseOrderMidTermEvaluationForCompany(companyId);
    if (result.isSuccess) {
      setLoading(false)
      setData([...result.data]);
...
export default POMidTermEvaluation;
