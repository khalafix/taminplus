...
const InboxInternalMessage = () => {
  ...
  const renderModalComponent = (type) => {
    return(
      <ShowMessage messageData={currentRow.body} id={currentRow.id}/>
    )
  };
  ...
  return (
    <PageContainer title={`${intl.formatMessage({ id: "listMessage" })}`}>
      <CPCard bodyStyle={{ padding: 10 }}>
        <AdvanceTable
          ...
          apiBuilder={userMessageService.getInbox}
          toolbar={ToolbarInit({
...
export default InboxInternalMessage;
