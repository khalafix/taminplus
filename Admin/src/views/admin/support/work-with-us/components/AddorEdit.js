...
const Add = ({
  ...
  const GetData = async () => {
    ...
    const result = await cooperationFormServices.getById(currentData.id);
    if (result.isSuccess) {
            ...
            <Row gutter={[8, 8]} className="mb-20">
              ...
              <Col xs={24} sm={12} md={8}>
                <CPInput
                  ...
                  name={"jobOpportunityTitle"}
                  type={"text"}
                  label={`${intl.formatMessage({ id: "jobOpportunity" })}:`}
                  placeholder={intl.formatMessage({ id: "jobOpportunity" })}
                  rules={[
                    {
                      message: <FormattedMessage id="requiredMessage" />,
                    },
