...
const Features = () => {
  ...
  const renderTitleModal = (type) => {
    switch (type) {
      case "add":
        return intl.formatMessage({ id: "add" })+' '+intl.formatMessage({ id: "features" });
      case "edit":
        return intl.formatMessage({ id: "edit" })+' '+intl.formatMessage({id:"features"});
      case "delete":
  ...
  const CreateFeature = async (data) => {
    setLoading(true);
    if (data.controlType == 104 && data.option=="") {
      message.error(intl.formatMessage({ id: "addNewItems" }));
      return 0
    }
    const result = await featureServices.add(data);
    if (result.isSuccess) {
  ...
  const UpdateFeature = async (data) => {
    ...
    const result = await featureServices.update(data);
    if (result.isSuccess) {
      message.success(result.message);
      setLoading(false);
      setOpenModal(false);
  ...
  const DeleteFeature = async (id) => {
    setLoading(true);
    const result = await featureServices.delete(id);
    if (result.isSuccess) {
      message.success(result.message);
      setLoading(false);
      setOpenModal(false);
  ...
  return (
    <PageContainer title={`${intl.formatMessage({ id: "listFeature" })}`}>
      <CPCard bodyStyle={{ padding: 10 }}>
        <AdvanceTable
          ...
          apiBuilder={featureServices.getAll}
          toolbar={ToolbarInit({
