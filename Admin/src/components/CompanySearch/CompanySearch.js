...
// UI Components
import { CPButton, CPInput, CPSelect, CPPagination, CPTreeSelect } from "components/CP";
...
const CompanySearch = ({submitAvls , index , bidderInterestRequestInquiryId}) => {
    const intl = useIntl();
    ...
    const getCategoryFeatures = async (categoryId) => {
        //setLoading(true);
        const result = await productCategoryServices.getFeaturesForFilter(categoryId);
        if (result.isSuccess) {
    ...
    const searchMatrialFeatures = async (params) => {
        ...
        for (var key in params) {
            if (params.hasOwnProperty(key)) {
                ...
                if (key.includes("featureHasValue") && params[key]) {
                    ...
                    list.push({ featureId: parseInt(featureId), value: params[key], title: "switch", controlType: 103 });
                }
                if (key.includes("featureValue") && params[key]) {
                    let featureId = key.replace("featureValue", "")
                    list.push({ featureId: parseInt(featureId), value: params[key], title: "text", controlType: 101 });
                }
                if (key.includes("featureOption") && params[key]) {
                    let featureId = key.replace("featureOption", "")
                    list.push({ featureId: parseInt(featureId), value: params[key], title: "tag", controlType: 104 });
                }
                if (key.includes("featureFrom") && params[key]) {
                    let featureId = key.replace("featureFrom", "")
                    list.push({ featureId: parseInt(featureId), value: params[key], title: "from", controlType: 102 });
                }
                if (key.includes("featureTo") && params[key]) {
                    let featureId = key.replace("featureTo", "")
                    list.push({ featureId: parseInt(featureId), value: params[key], title: "to", controlType: 102 });
                }
            }
        }
        ...
        if (list.length > 0) {
            const result = await productCategoryServices.searchFeaturesForCompany(model);
            if (result.isSuccess) {
                setData(result.data);
                setTotal(result.total);
                setPages(Math.ceil(result.total / size));
    ...
    const renderSearchPanelForMatrialFeatures = (items) => {
        if (items?.length > 0) {
            ...
            return (<Col xs={24} sm={24} md={24} className="mb-20 mt-20">
                <strong>
                                ...
                                <Row gutter={[8, 8]} className="mb-10 mt-20 ">
                                    ...
                                    <Col xs={24} sm={12} md={6}>
                                        <CPTreeSelect
                                            ...
                                            onChange={async (value) => await handleChangeCategory(value)}
                    ...
                    <Col sm={24} md={24} xs={24}>
                        {
                            dataFeatures?.length > 0 ?
                                <CPPanel
                                    header={
                                        intl.formatMessage({ id: "advanceSearchForMatrialFeatures" })
                                    } bordered>
                                    <Form form={featureForm} name="searchMatrialFeatures" onFinish={searchMatrialFeatures} layout="vertical">
                                        ...
                                        <Row gutter={[8, 8]} >
                                            ...
                                            <Space>
                                                ...
                                                <CPButton
                                                    htmlType="submit">
                                                    <span className="btn-icon">{iconMap["AiOutlineRest"]}</span>
                ...
                <Row>
                    <Col span={12}>
                        <Space>
                            <CPButton
                                type="primary"
                                onClick={() => submitAvls(selectedRowKeys,  index ,data,  bidderInterestRequestInquiryId)}
                            >
                                <span>
                                    <FormattedMessage id="setSelectedCompany" />
                                </span>
                            </CPButton>
                        </Space>
                    </Col>
